...
exports.changeStatus = async (req, res) => {
  try {
    const data = await service.changeStatus(req.user.id, req.params.id, req.body.status);
    return res.json({ success: true, data });
  } catch (error) {
    return res.status(error.statusCode || 500).json({ error: error.message });
  }
...
exports.getVendorRFQs = async (req, res) => {
  try {
    const data = await service.getVendorRFQs(req.params.id);
    return res.json({ success: true, data });
  } catch (error) {
    return res.status(error.statusCode || 500).json({ error: error.message });
  }
...
exports.getVendorQuotations = async (req, res) => {
  try {
    const data = await service.getVendorQuotations(req.params.id);
    return res.json({ success: true, data });
  } catch (error) {
    return res.status(error.statusCode || 500).json({ error: error.message });
  }
