...
export default function ResetPasswordPage() {
  ...
  const { register, handleSubmit, formState: { isSubmitting } } = useForm({ defaultValues: { password: '' } });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-bg p-6">
      <div className="panel w-full max-w-md p-8">
        ...
        <h1 className="mt-2 text-2xl font-bold">Create new password</h1>
        <form className="mt-6 space-y-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <button className="btn-primary w-full" disabled={isSubmitting || !token}>{isSubmitting ? 'Saving...' : 'Reset password'}</button>
        </form>
